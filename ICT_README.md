...
├── ict.py              — Lớp 4: is_swept, grade_fvg, is_shift (stub)
...
| 4 | `ict.py` | Swept, FVG graded, Shift/MSS |
...
facts = build_facts(parser, lookback=10)
# {
#   "n_candles": 20,
#   "swept"    : [...],
#   "fvg"      : [...],
...
|---|---|---|
| Phân phối `depth` của Swept | ❌ Chưa chạy | Q-score graded cho Swept (độ sâu sweep) |
...
|--------|-----------|---------|
| `candle.py` | ✅ Xong | 61/61 test pass |
| `parser.py` | ✅ Xong | |
| `basic.py` | ✅ Xong | Ngưỡng ĐÃ XÁC NHẬN bằng thống kê thật (N=19.4M nến XAUUSD M1) |
| `structure.py` | ✅ Xong | |
| `ict.py::is_swept` | ✅ Xong | 10/10 golden test pass (8 gốc + 2 bổ sung) |
| `ict.py::grade_fvg` | ⚠️ Nháp | Logic có, chưa chốt behavior `fill_pct` (KNOWN ISSUE) |
| `ict.py::is_shift` | ❌ Stub | `raise NotImplementedError` — phải viết `test_shift.py` trước |
| `relations.py` | ✅ Xong | Tie-breaking `SAME_CANDLE` chưa có rule ưu tiên (by design) |
| `facts.py` | ✅ Xong | `shift` field rỗng list cho tới khi `is_shift` xong |
...
**Giai đoạn 2 (hiện tại):** Chạy thống kê phân phối trên data CSV thật
(script ở trên), điền số thật vào các hằng số trong `basic.py`, cập nhật
comment "placeholder" thành "đã xác nhận".

**Giai đoạn 3:** Viết `tests/test_shift.py` (test-first) để định nghĩa
behavior `is_shift()` trước khi cài logic, đặc biệt phân biệt:
- BOS (Break of Structure): phá swing cùng hướng trend hiện tại
- CHoCH (Change of Character): phá swing ngược hướng trend
- Near-miss: wick xuyên qua swing nhưng giá **đóng cửa** không vượt → không tính

**Giai đoạn 4:** `build_relations()` đã xong logic, cần chạy thử trên
output thật của Giai đoạn 3 (không chỉ unit test tay).

**Giai đoạn 5:** Triển khai `render.py` và `validate.py` sau khi
`is_shift()` và `build_relations()` đã có golden test đầy đủ.
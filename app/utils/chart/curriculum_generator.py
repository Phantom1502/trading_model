...
=======================
Sinh dataset TEXT THUẦN (continued pretraining) theo curriculum 7 tầng,
dạy model nhận thức về phân tích nến ICT theo đúng thứ tự cơ bản → phức tạp:
    ...
    Tầng 6 — MẪU HÌNH NẾN     : Pin Bar (Hammer/Shooting Star) và Engulfing,
                                kèm gợi ý xác suất (không khẳng định chắc) hướng giao dịch
...
RANDOM HÓA: không phải mẫu nào cũng cần giới thiệu đủ 7 tầng. Dùng
...
Cách dùng:
    ...
    full_text   = gen.generate_full_curriculum()                  # đủ 7 tầng
...
class CurriculumGenerator:
    """
    Sinh text pretrain theo curriculum 7 tầng từ 1 CandleParser đã parse sẵn.
    """
    ...
    def __init__(self, parser: CandleParser):
        ...
        # Danh sách method theo ĐÚNG thứ tự tầng 0→6, dùng chung cho
...
"""
    ...
    # ══════════════════════════════════════════════════════════════
    # TẦNG 6 — MẪU HÌNH NẾN (PIN BAR, ENGULFING) + GỢI Ý XU HƯỚNG
    # ══════════════════════════════════════════════════════════════
    def layer6_candle_patterns(self) -> str:
        """
        Dạy các mẫu hình nến đơn giản (price action patterns) và gợi ý xu hướng
        thường gặp đi kèm — KHÔNG khẳng định chắc chắn BUY/SELL, chỉ nêu xu hướng
        thống kê thường thấy, vì mẫu hình chỉ là một gợi ý xác suất, không phải
        quy luật tuyệt đối.

        Bao gồm:
        - Mẫu hình 1 nến : Hammer, Shooting Star (dạng Pin Bar)
        - Mẫu hình 2 nến : Bullish Engulfing, Bearish Engulfing

        Các mẫu hình này được phát hiện ĐỘC LẬP theo đúng hình dạng nến,
        không phụ thuộc vào việc nến đó có đang ở Swing High/Low hay không.
        """
        lines: List[str] = []
        lines.append("=== KHÁI NIỆM: MẪU HÌNH NẾN (PRICE ACTION PATTERNS) ===\n")
        lines.append(
            "Ngoài việc xét từng cây nến riêng lẻ hay quan hệ giữa nhiều nến như Swing "
            "và FVG, một số TỔ HỢP HÌNH DẠNG nến cụ thể cũng thường được dùng làm gợi ý "
            "cho hướng đi tiếp theo của giá. Đây là các mẫu hình PHỔ BIẾN trong phân tích "
            "price action, gồm mẫu hình 1 nến và mẫu hình 2 nến.\n"
        )
        lines.append(
            "- Hammer (mẫu 1 nến): thân nến nhỏ, có bóng dưới (lower wick) rất dài, "
            "hầu như không có bóng trên. Mẫu hình này thường xuất hiện sau một đợt giảm "
            "giá, và thường gợi ý khả năng giá có thể đảo chiều TĂNG — có thể xem xét "
            "BUY, nhưng đây chỉ là một gợi ý xác suất, không phải quy luật chắc chắn.\n"
            "- Shooting Star (mẫu 1 nến): thân nến nhỏ, có bóng trên (upper wick) rất "
            "dài, hầu như không có bóng dưới. Mẫu hình này thường xuất hiện sau một đợt "
            "tăng giá, và thường gợi ý khả năng giá có thể đảo chiều GIẢM — có thể xem "
            "xét SELL, nhưng cũng chỉ là một gợi ý xác suất.\n"
            "- Bullish Engulfing (mẫu 2 nến): một nến giảm nhỏ theo sau bởi một nến tăng "
            "có thân lớn hơn, 'nuốt trọn' toàn bộ thân của nến giảm trước đó. Mẫu hình "
            "này thường gợi ý lực mua đang áp đảo, có thể xem xét BUY.\n"
            "- Bearish Engulfing (mẫu 2 nến): một nến tăng nhỏ theo sau bởi một nến giảm "
            "có thân lớn hơn, nuốt trọn toàn bộ thân của nến tăng trước đó. Mẫu hình này "
            "thường gợi ý lực bán đang áp đảo, có thể xem xét SELL.\n"
        )
        lines.append(
            "Lưu ý quan trọng: các mẫu hình nến chỉ là gợi ý mang tính XÁC SUẤT dựa trên "
            "quan sát thống kê thường gặp, KHÔNG phải là quy luật chắc chắn. Một mẫu hình "
            "Hammer không có nghĩa là giá chắc chắn sẽ tăng, mà chỉ là một tín hiệu nên "
            "được kết hợp thêm với các khái niệm khác (như Swing, FVG) để tăng độ tin cậy.\n"
        )
        ...
        for i in range(self.n):
            ...
            if pin == "HAMMER":
                lines.append(
                    f"Cây nến thứ {ordinal} {c.tag()} có bóng dưới dài và thân nhỏ "
                    f"→ đây là mẫu hình HAMMER, thường gợi ý khả năng đảo chiều TĂNG "
                    f"(có thể xem xét BUY)."
                )
                found_any = True
            elif pin == "SHOOTING_STAR":
                lines.append(
                    f"Cây nến thứ {ordinal} {c.tag()} có bóng trên dài và thân nhỏ "
                    f"→ đây là mẫu hình SHOOTING STAR, thường gợi ý khả năng đảo chiều "
                    f"GIẢM (có thể xem xét SELL)."
                )
                found_any = True

            engulf = self.parser.is_engulfing(i)
            if engulf == "BULLISH_ENGULFING":
                prev = self.candles[i - 1]
                lines.append(
                    f"Cây nến thứ {i} {prev.tag()} và cây nến thứ {ordinal} {c.tag()} "
                    f"tạo thành mẫu hình BULLISH ENGULFING (nến sau nuốt trọn thân nến "
                    f"trước) → thường gợi ý lực mua đang áp đảo (có thể xem xét BUY)."
                )
                found_any = True
                ...
                lines.append(
                    f"Cây nến thứ {i} {prev.tag()} và cây nến thứ {ordinal} {c.tag()} "
                    f"tạo thành mẫu hình BEARISH ENGULFING (nến sau nuốt trọn thân nến "
                    f"trước) → thường gợi ý lực bán đang áp đảo (có thể xem xét SELL)."
                )
                found_any = True

        if not found_any:
            lines.append("Trong chart này không tìm thấy mẫu hình nến (Pin Bar/Engulfing) rõ ràng.")

        return "\n".join(lines).strip() + "\n"

    # ══════════════════════════════════════════════════════════════
    # GHÉP TOÀN BỘ CURRICULUM (ĐỦ 7 TẦNG)
    # ══════════════════════════════════════════════════════════════
    def generate_full_curriculum(self, include_chart_header: bool = True) -> str:
        """
        Sinh toàn bộ text pretrain theo đúng thứ tự 7 tầng, từ khái niệm cơ bản
    ...
    def generate_layers_separately(self) -> List[str]:
        """
        Trả về list 7 đoạn text riêng biệt (1 đoạn / 1 tầng) thay vì 1 văn bản dài.
    ...
    # ══════════════════════════════════════════════════════════════
    # RANDOM SUBSET — không phải sample nào cũng cần đủ 7 tầng
    # ══════════════════════════════════════════════════════════════
    def generate_random_subset(
        ...
        """
        Sinh 1 đoạn text pretrain từ MỘT TỔ HỢP TẦNG NGẪU NHIÊN, không cần đủ 7 tầng.
        ...
        max_layers  : số tầng tối đa (mặc định = num_layers, tức có thể ra đủ 7 tầng)
...
def build_pretrain_dataset(
    ...
    mode       : "full"   → mỗi chart sinh 1 văn bản đầy đủ 7 tầng (liên tục)

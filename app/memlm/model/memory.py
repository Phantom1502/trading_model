...
Thiết kế:
    ...
    - Read  : x_post tra vấn memory_new
...
Gradient path (khép kín trong 1 forward pass):
    loss → gated_out → m_out   → read  ← memory (detach ở block, dừng ở đây)
    loss → gated_out → gate    → gate_proj([x_post, m_out]) ✓
    loss → gated_out → m_out   → read  → (write → delta → memory_new, nhưng
           memory_new chỉ được dùng ở batch SAU đã detach → write nhận gradient
           gián tiếp qua signal từ batch sau, không trực tiếp trong batch này)
...
class MemoryLayer(nn.Module):
    ...
    def forward(
        self,
        x_post : torch.Tensor,   # (B, T, D) — đã qua causal self-attn
        ...
        """
        # ── READ: x_post tra vấn memory_cũ ──────────────────────────────────
        m_normed = self.norm_m(memory)                     # (B, num_slots, D)
        ...
        memory_new = self.alpha * memory + (1 - self.alpha) * delta

...
## 8. Vai trò GPT trong sinh data — chỉ diễn đạt, không quyết định sự thật

| Vai trò | Ai làm | Lý do |
|---|---|---|
| Xác định có pattern hay không, Q-score bao nhiêu, sequence đúng/sai | Giải thuật (bắt buộc, không thương lượng) | Đây là target dùng đo tương quan outcome ở Giai đoạn 4 (mục 11) — nếu sai từ gốc, mọi thống kê sau đều vô nghĩa, và sai một cách hệ thống (khó phát hiện hơn lỗi random) |
| Viết lại JSON fact thành văn xuôi đa dạng | GPT | Tăng diversity cách diễn đạt, tránh model học thuộc 1 khuôn câu cố định |

**Prompt khung cho GPT** (tham khảo, không phải code):
```
Cho dữ kiện sau (đã tính chính xác, KHÔNG được đổi số liệu):
{JSON fact}
Viết 1 đoạn mô tả tự nhiên (2-3 câu) theo tư duy ICT, PHẢI giữ nguyên số liệu,
KHÔNG được thêm pattern không có trong dữ kiện, KHÔNG suy đoán thêm thông tin
ngoài dữ kiện. Văn phong có thể đổi, số liệu không được đổi.
```

**Rủi ro cần kiểm soát:** GPT có thể tự thêm chi tiết không có trong fact → luôn cần script validate đối chiếu câu sinh ra với JSON gốc, loại mẫu lệch. Không review tay từng câu — chạy batch rồi validate tự động.
